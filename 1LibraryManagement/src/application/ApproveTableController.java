...
public class ApproveTableController {
	...
	public void acceptSelected(ActionEvent event) throws SQLException {
		...
		Student student = new Student(studentForApprove.getStudentId(), studentForApprove.getStudentFirstName(),
				...
				LocalDate.now().toString(), emptyArray, emptyArray, emptyArray, 0);
		String sql = "INSERT INTO students (studentId, studentFirstName, studentLastName, studentPhoneNumber,"
				...
				+ " studentdeliverystatus, studentreturndate)"
				+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		addToDatabase(sql, student.getStudentId(), student.getStudentFirstName(), student.getStudentLastName(),
				...
				student.getStudentDeliveryStatus(), student.getStudentLateFee());
	...
	private void addToDatabase(String sql, long studentId, String studentFirstName, String studentLastName,
			...
			String[] studentBorrowedBooks, String[] studentReturnDate, String[] studentDeliveryStatus,
			int studentLateFee) {
		try {
			...
			String borrowedBooksJson = Arrays.toString(studentBorrowedBooks); 
			String returnDateJson = Arrays.toString(studentReturnDate);
			String deliveryStatusJson = Arrays.toString(studentDeliveryStatus);
			...
			stmt.setString(9, borrowedBooksJson);
			stmt.setString(10, returnDateJson);
			stmt.setString(11, deliveryStatusJson);
			stmt.executeUpdate();
